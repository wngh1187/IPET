...
class ModelTrainer:
    ...
    def run(self):
        ...
        self.best_acc = {}
        self.best_acc['val'] = -np.inf
        self.best_acc['eval'] = -np.inf
    ...
    def test(self, epoch, mode, dataloader):
        ...
        if self.args.flag_parent:
            ...
            if acc > self.best_acc[mode]:
                self.best_acc[mode] = acc
                self.logger.log_metric('Bestacc_{}'.format(mode), acc, epoch_step=epoch)
