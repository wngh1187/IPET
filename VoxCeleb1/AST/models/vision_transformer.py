...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        q, k, v = qkv[0], qkv[1], qkv[2]   # make torchscript happy (cannot use tensor as tuple) B, head, N, C

        attn = (q @ k.transpose(-2, -1)) * self.scale # B, head, N, C @ B, head, C, N -> B, head, N, N
        attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn)

        x = (attn @ v).transpose(1, 2).reshape(B, N, C) # B, head, N, N @ B, head, N, C -> B, head, N, C -> B, N, head, C
        x = self.proj(x)
        x = self.proj_drop(x)
        return x
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        if args.adapter:
            self.adaptmlp = Adapter(args, dropout=0.1, bottleneck=args.adapter_hidden_dim,
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0., hybrid_backbone=None, norm_layer=None, args = None):
        """
        Args:
        ...
        self.blocks = nn.ModuleList([
            Block(
                ...
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, args=args)
...
def _create_vision_transformer(variant, pretrained=False, distilled=False, args=None, **kwargs):
    ...
    model = model_cls(img_size=img_size, num_classes=num_classes, representation_size=repr_size, args=args, **kwargs)
