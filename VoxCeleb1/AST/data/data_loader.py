...
class Kaldi_Fbank():
    ...
    def __call__(self, x, sr, x2 = None):
        ...
        if not(self.is_test) or self.args.nb_seg == 1:
            # cut and pad
            if p > 0:
            ...
            if p > 0:
                nb_dup = int(self.args.frame_length / n_frames) + 1
                fbank = torch.tile(fbank, (nb_dup,1))
            
            buffer = []
            index = np.linspace(0, fbank.shape[0] - self.args.frame_length, self.args.nb_seg) 
            
            for i in range(len(index)):
                start = int(index[i])
                end = start + self.args.frame_length    
                buffer.append(fbank[start:end, :])
            
            fbank = torch.cat(buffer).view(self.args.nb_seg, self.args.frame_length, -1)
...
class TestSet(td.Dataset):
    ...
    def __init__(self, args, dataset, classes_labels):
        ...
        self.fbank = Kaldi_Fbank(args, is_test = True)
