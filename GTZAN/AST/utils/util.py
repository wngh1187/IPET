...
def rand_short_crop(x, size, dim=-1):
    """random crop for data augmentation
    ...
    Param
        ...
        dim     - target dimension
    """
    target_size = random.randint(16000, size)
    if x.size(dim) < target_size:
        x = duplicate(x, target_size, dim)
    index = random.randint(0, x.size(dim) - target_size)
    x = subtensor(x, index, target_size, dim)
...
def init_weights(m):
    ...
    elif isinstance(m, nn.BatchNorm1d) or isinstance(m, nn.BatchNorm2d) or isinstance(m, nn.InstanceNorm1d) or isinstance(m, nn.LayerNorm):
        pass
    else:
        if hasattr(m, 'weight'):
