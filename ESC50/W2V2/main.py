...
import models.W2V2 as wav2vec2
...
def run(process_id, args):
    ...
    # results per fold and per epoch (acc, mAP, ...)
    trainer.results = np.zeros([args.fold, args.epoch, 10])

    for fold in range (1, args.fold+1):
        trainer.fold  = fold

        trainer.dataset = Datasets(args, fold)
        
        args.num_classes = len(trainer.dataset.classes_labels)
        
        # data loader
        loaders = data.get_loaders(args, trainer.dataset)
        trainer.train_set, trainer.train_set_sampler, trainer.train_loader, trainer.evaluation_set, trainer.evaluation_loader = loaders[0], loaders[1], loaders[2], loaders[3], loaders[4]
        ...
        wav2vec2.args = args
        model = wav2vec2.Model.from_pretrained(
            args.model_type,
            from_tf=bool(".ckpt" in args.model_type),
            config=config,
            revision="main",
        ...
        classification_head = modelue(
            embd_dim = 768 *2, #concat mean and std,
    ...
    if args.flag_parent:
        ...
        for i in range(args.fold):
            print(trainer.results[i, :, 0])
            print(trainer.results[i, :, 1])
            best_acc_epoch = np.argmax(trainer.results[i, :, 0])
            best_map_epoch = np.argmax(trainer.results[i, :, 1])
            best_acc_fold.append(trainer.results[i, best_acc_epoch, 0])
            best_map_fold.append(trainer.results[i, best_map_epoch, 1])
        logger.log_metric('best_final_mean_acc', np.mean(best_acc_fold), epoch_step = best_acc_epoch)
        logger.log_metric('best_final_mean_map', np.mean(best_map_fold), epoch_step = best_map_epoch)
